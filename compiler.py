...
def ids(lex,qual):
    ...
    # TODO
    global mem_address
    identifiers[lex.current[1]]=(mem_address,qual)
    mem_address+=1
    lex.goNext()
    output+=lex.current_token_header()
    output+=ids_prime(lex,qual)
    return output

# <IDs Prime> -> Epsilon | ,<IDs>
def ids_prime(lex,qual):
    try:
    ...
    if lex.current[1] != "scan":
        ...
        if lex.current[0] == "Identifier":
            if identifiers[lex.current[1]]:
                ...
                generate_instruction("POPM",identifiers[lex.current[1]][0])
                lex.goNext()
                output += lex.current_token_header()
            else:
                print("ERROR")
...
def syntax_analyzer(input_file,output_file):
    ...
    try:
        ...
        with open(output_file, 'w') as file:
            ...
            file.write("\nIdentifier    Memory Addr   Type\n")
            print("\nIdentifier    Memory Addr   Type\n")
            for x in identifiers.keys():
                print(f"{x}".ljust(14),end="")
                print(f"{identifiers[x][0]}".ljust(14),end="")
                print(f"{identifiers[x][1]}".ljust(14))
                file.write(f"{x}".ljust(14))
                file.write(f"{identifiers[x][0]}".ljust(14))
                file.write(f"{identifiers[x][1]}".ljust(14))
                file.write('\n')
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Error =====\nUsage: python3 compiler.py <input_file> <output_file>")
        sys.exit(1)
